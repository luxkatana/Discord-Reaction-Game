...
async def start_reaction(ctx: discord.ApplicationContext):
    ...
    async def on_timeout() -> None:
        ...
        await m.edit_original_message(view=None, embed=discord.Embed(
            title="Cancelled",
    ...
    for emoji in EMOJIS:
        if emoji == rand_btn:
            async def now_callback(interaction: discord.Interaction) -> None:
                ...
                success_embed = discord.Embed(title=f"Time: {difference:.3f} seconds", description=f"It took {difference:.3f} seconds to click on the button", colour=discord.Color.green())
                success_embed.set_footer(text="From {}".format(ctx.author))
                
                await interaction.response.send_message(embed=success_embed)

                await m.delete_original_message()
                v.stop()
            ...
            async def fail_callback(interaction: discord.Interaction) -> None:
                if interaction.user != ctx.user:
                    await interaction.response.send_message("This was not meant for you", ephemeral=True)
                    return
                failed_embed = discord.Embed(title="Failed", description=f"You Failed bruh! the button was {rand_btn}", colour=discord.Color.red())
    ...
    embed =discord.Embed(title="Reaction game", description=f"Lets do a reaction game\nthe emoji that i choose is...\n click the button with the {rand_btn} below", color=discord.Color.random())
    m = await ctx.respond(embed=embed, view=v)
